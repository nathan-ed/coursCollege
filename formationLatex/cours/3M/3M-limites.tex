...
\begin{remarque}
	\tcblower
	On utilise une notation qui n'est pas rigoureuse en écrivant $«~\dfrac{1}{0}~»$. On va dès maintenant également une notation pour travailler avec l'infini. Cette algèbre de l'infini peut être formlisée rigoureusement, mais cela dépasse le cadre de ce cours et nous admettrons les propriétés suivantes. 
\end{remarque}
...
\begin{remarque}
	\tcblower
	Une limite \(\displaystyle{\lim_{x\to\pm\infty}f(x)=\ell}\) s'approche dans un premier temps à l’aide des formules de l'algèbre de l'infini, du théorème \ref{thm:limel} et du théorème \ref{thm:limprop} qui restent valables dans ces situations. Si le calcul ne peut pas être traité avec ces outils, on verra plus loin que d’autres techniques peuvent être appliquées.
\end{remarque}
\begin{exemple}
