...
\section{Définition et exemples graphiques}
\begin{exemple}
	\tcblower
...
\addplot[blue, mark=*, mark options={fill=black, scale=1.5}] coordinates {(0,0.75)};
...
\end{tikzpicture}
	...
	\[\displaystyle{\lim_{x\rightarrow -1^-}f(x)=}\ligne{1} \quad \text{et} \quad \displaystyle{\lim_{x\rightarrow -1^+}f(x)=}\ligne{1}\]
...
\lim_{x\to a}f(x)=f(a).
\]
Dans tous ces cas « simples », le comportement de \(f\) près de \(a\) est le même que celui de \(f\) en \(a\). Ce n’est donc pas pour ces fonctions que l’«~outil limite~» est pertinent, mais pour celles de la section suivante…

Dans la partie \ref{sec:continuite}, nous reviendrons sur cette remarque qui nous permettra d’appréhender une notion très importante, {\bfseries la continuité}.
\end{remarque}
...
\begin{remarque}
	\tcblower
	Une limite \(\displaystyle{\lim_{x\to\pm\infty}f(x)=\ell}\) s'approche dans un premier temps à l’aide des formules de l'algèbre de l'infini des théorèmes des parties \ref{sec:limel} et \ref{sec:proplim} qui restent valable dans cette situation. Si le calcul ne peut pas être traité avec ces outils, on verra plus loin que d’autres techniques peuvent être appliquées.
\end{remarque}
\begin{exemple}
	\tcblower
	$\begin{aligned}
...
\begin{exemple}
	...
	$
\end{exemple}
\section{Lever les indéterminations}

Il arrive que toutes les méthodes vues jusque-là pour calculer des limites ne soient pas utilisables. 
...
\end{tikzpicture}
	\end{center}
\end{exemplesuite}
\section{Asymptotes verticales et horizontales}
...
\begin{remarque}
	\tcblower
	Ces résultats sont des corollaires des théorèmes \ref{thm:limel} et \ref{thm:limprop}. 
\end{remarque}
\newpage
%\section{Activités}
%\begin{activite}
