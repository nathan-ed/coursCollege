...
\qrcode[height=2.5cm]{https://example.com/racines} &
\checkBox \\
%------------ Algèbre ------------------------------------------------
\SetCell[r=2]{l, bg=green!10} {\textbf{Algèbre}} &
Factoriser un polynôme en déterminant ses racines &
\qrcode[height=2.5cm]{https://example.com/factoriser} &
\checkBox \\
&
Effectuer la division polynomiale &
\qrcode[height=2.5cm]{https://example.com/division} &
\checkBox \\
%------------ Fonctions ----------------------------------------------
\SetCell[r=3]{l, bg=orange!10} {\textbf{Fonctions}} &
Déterminer le domaine de définition d'une fonction &
\qrcode[height=2.5cm]{https://example.com/domaine} &
\checkBox \\
&
Déterminer les zéros d'une fonction &
\qrcode[height=2.5cm]{https://example.com/zeros} &
\checkBox \\
&
Représenter graphiquement une fonction du second degré &
\qrcode[height=2.5cm]{https://example.com/graphique} &
\checkBox \\
\end{tblr}
...
$f$ est continue en $a$ ssi il existe un voisinage de $a$ tel que « une représentation graphique de $f$ dans ce voisinage peut être dessinée sans lever le crayon ».  
\end{definition}
\begin{remarque}
	\tcblower
	Cette définition peut être formalisée, mais cela dépasse le cadre de ce cours.
\end{remarque}
...
\tcblower
Si $f : I \to \mathbb{R}$ est une fonction continue sur $[a,b]$, alors la fonction $f$
\begin{itemize}
	\item est bornée sur $[a,b]$
	\item possède un minimum $m$ et un maximum $M$ sur $[a,b]$
	\item prend au moins une fois toutes les valeurs comprises entre $m$ et $M$.
\end{itemize}
En d'autres termes, $f([a,b]) = [m,M]$
\end{thm}
\begin{coro}
	\tcblower
	Soit $f$ une fonction continue sur un intervalle $[a;b]$ si 
	\[f(a)\geq 0 \text{ et } f(b) \leq 0 \text{ {\bfseries ou} } f(a)\leq 0 \text{ et } f(b)\geq 0,\]
	alors $f$ s'annule sur $[a;b]$, c'est-à-dire qu'il existe $c\in [a;b]$ tel que $f(c)=0$. 
\end{coro}
\begin{remarque}
	\tcblower
...
\insertexo{1M-59swy}{false}{exo}{true}
\insertexo{1M-qp2mw}{true}{exo}{true}

\section{Limite en un nombre}
...
\task Utilisons alors des tableaux de valeurs :
\begin{itemize}
...
\begin{exemple}
	\tcblower
	Dans l'exemple \ref{ex:fmorceaux}, la fonction $f$ n'admet pas de limite en $1$, car par la contraposée du théorème \ref{thm:existence} la limite à gauche et la limite à droite ne sont pas égales.  
\end{exemple}
\begin{exemple}
		\tcblower
...
On peut à présent donner une définition plus formelle de la continuité. 
...
=« \dfrac{1}{(0^-)^2} »=\ligne{1},\quad
\lim_{x\to0^+}\dfrac1{x^2}
=«\dfrac{1}{(0^+)^2}»=\ligne{1},
\]
donc
\[
	\lim_{x\to0}\dfrac1{x^2}=\ligne{1}.
\]
\end{exemple}
...
\]
\[
	\text{donc }\lim_{x\to -2}\dfrac{-2}{(x+2)^2}\ligne{2}.
\]
\end{exemple}
...
\]
\[
	\text{donc }\lim_{x\to 1}\dfrac{x-2}{x^2+3x-4}\,\ligne{3}.
\]
\end{exemple}
...
\begin{definition}
	Type $«~\dfrac{0}{0}~»$
	\tcblower
	Une limite du type quotient \(\displaystyle \lim_{x\to a}\dfrac{f(x)}{g(x)}\) pour laquelle on a \(\displaystyle \lim_{x\to a}f(x)=0\) et \(\displaystyle \lim_{x\to a}g(x)\neq0\) est appelée limite de type \(«~\displaystyle \dfrac{0}{0}~»\). 
\end{definition}
\begin{remarque}
	\tcblower
Il y a trois cas pour les limites « $\dfrac{1}{0}$ »~:
\begin{description}
...
\begin{definition}
	...
	verticale
	\tcblower
On dit qu'une droite verticale d'équation $x = a$ est une \textbf{asymptote verticale} de la fonction $f$ si l'une au moins des conditions suivantes est satisfaite~:
...
\begin{exemple}
	lecture graphique d'asymptotes
\tcblower	
$x = -1$ et $x = 1$ sont deux asymptotes verticales de la fonction ;

$y = 2$ est une asymptote horizontale de la fonction.
\end{exemple}

\begin{remarque}
	$\dfrac{\sin(x)}{x}$
\tcblower
Il est possible d'intersecter une asymptote. Par exemple, $y = 0$ est une asymptote horizontale de la fonction la fonction $f$ définie par $f(x) = \dfrac{\sin(x)}{x}$
\end{remarque}
...
\end{exemple}
\subsection{Exercices}
\subsubsection{Définition de la limite}
\insertexo{1M-4dptb}{true}{exo}{true}
\insertexo[exo:p1f]{1M-g4gbb}{true}{exo}{true}
\subsubsection{Limites de fonctions élémentaire et propriétés des limites}
\insertexo[exo:p2d]{1M-tecfn}{true}{exo}{true}
\insertexo{1M-f7dft}{true}{exo}{true}
\insertexo{1M-b3p6g}{true}{exo}{true}
\subsubsection{Retour sur la continuité}
Vous avez à présent les outils nécessaires pour résoudre ces exercices.
\insertexo{1M-pp62m}{false}{exo}{true}
\insertexo{1M-hsytn}{false}{exo}{true}
\insertexo{1M-n9sxp}{false}{exo}{true}
\insertexo{1M-u44h9}{false}{exo}{true}
...
\insertexo{1M-89av7}{true}{exo}{true}
...
On dit que la limite de \(f(x)\) quand \(x\) tend vers $+\infty$ est égale à \(L\) si \(f(x)\) s'approche « aussi près qu'on veut » de \(L\) lorsque \(x\) devient infiniment grand (avec signe positif). On note $\lim_{x\to+\infty}f(x)=L$.

On dit que la limite de \(f(x)\) quand \(x\) tend vers $-\infty$ est égale à \(L\) si \(f(x)\) s'approche « aussi près qu'on veut » de \(L\) lorsque \(x\) devient infiniment grand (avec signe négatif). On note $\lim_{x\to-\infty}f(x)=L$.
\end{definition}
...
Nous avons déjà traité les indéterminations du type $« ~\dfrac{\star}{0}~»$. Dans cette section, nous traitons les indéterminations qui font intervenir l'infini. 
...
\]
est appelée {\bfseries indétermination} de « type \(\displaystyle\dfrac{0}{0}\) ».

Les différents types d’{\bfseries indéterminations} sont (liste non exhaustive) :
\[
...
\begin{definition}
	...
	\tcblower
On dit qu'une doite horizontale d'équation $y = b$ est une \textbf{asymptote horizontale} de la fonction $f$ à si au moins des conditions suivantes est satisfaite~:
...
\begin{exemple}
	\tcblower
	Déterminer les asymptotes horizontales de la fonction $f$ définie par (et déjà étudiée à l'exemple \ref{ex:asver})~:
\[f(x) = \dfrac{x^2 - x}{x^2 + 3x - 4}\]
...
&= \lim_{x \to +\infty} \dfrac{1}{1 + \dfrac{4}{x}} = \dfrac{1 - 0}{1 + 0}\\
&= \dfrac{1}{1} = 1
\end{aligned}$
...
\begin{definition}
	asymptote oblique
	\tcblower
Une droite $d$ d'équation $d(x) = ax + b$ avec $a \neq 0$ est appelée \textbf{asymptote oblique} de la fonction $f$ si l'une au moins des conditions suivantes est satisfaite :
\[
\displaystyle\lim_{x \to +\infty} f(x) - d(x) = 0 \quad \text{et/ou} \quad \displaystyle\lim_{x \to -\infty} f(x) - d(x) = 0
\]
\end{definition}
\begin{exemple}
asymptote oblique
\tcblower	
La fonction ci-contre admet $y = -x + 2$ comme asymptote oblique à $+\infty$, alors que la fonction ci-dessous admet $y = x + 2$ comme asymptote oblique à $+\infty$ et $y = -2$ comme asymptote oblique à $-\infty$.
\end{exemple}
...
Ainsi, 
\begin{align*}
f(x) &= \dfrac{x^2}{x + 2} = \dfrac{(x + 2)(x - 2) + 4}{x + 2} = \dfrac{(x + 2)(x - 2)}{x + 2} + \dfrac{4}{x + 2} \\
&= x - 2 + \dfrac{4}{x + 2}
\end{align*}
...
\begin{thm}[label=thm:obli]
	...
	\tcblower
Si $\displaystyle\lim_{x \to \pm\infty} \dfrac{f(x)}{x} = a$ et $\displaystyle\lim_{x \to \pm\infty} (f(x) - ax) = b$ avec $a \in \mathbb{R}^*$ et $b \in \mathbb{R}$, alors la droite $d$ définie par $d(x) = ax + b$ est une asymptote oblique de la fonction $f$ à $\pm\infty$.
\end{thm}
\begin{remarque}
	\tcblower
	La contraposée du théorème \ref{thm:obli} est aussi vraie.
\end{remarque}
...
\insertexo{1M-us8px}{false}{exo}{true}
...
Théorie~:
\begin{itemize}
	\item Sesamath 3e année, 2023--2024
\end{itemize}
Exercices~:
\begin{itemize}
	\item Sesamath 3e année, 2023--2024
