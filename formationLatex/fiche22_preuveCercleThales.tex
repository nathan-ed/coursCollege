...
\usepackage[fiche,niveau={1MA1},nfiche={22}, annee={Emilie-Gourd, 2024--2025},
auteur={ns},theme={Géométrie -- Preuves du cercle de Thalès}]{packages/bandeaux}
...
Si un triangle $ABC$ inscrit dans un cercle est rectangle en $C$ alors $AB$ est un diamètre du cercle.  
\end{thm}
\begin{proof}
\begin{minipage}[t]{0.5\textwidth}{
\vspace{0pt}
...
Les deux médiatrices $d_{OA'}$ et $d_{OC'}$ s'intersectent au point $O$ qui est le centre du cercle circonscrit dont le côté $AC$ est le rayon (car c'est l'hypoténuse). 
}
\end{minipage}
\hfill
\begin{minipage}[t]{0.4\textwidth}{
