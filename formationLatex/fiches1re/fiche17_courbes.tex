...
\newcommand{\path}{../}
\usepackage[fiche,niveau={1MA1},nfiche={17}, annee={Emilie-Gourd, 2024--2025}, auteur={ns},theme={Fonctions -- Paraboles}]{\path packages/bandeaux}
...
\begin{defi}[parabole]
	Soit une fonction $f$ du deuxième degré, $f(x)=ax^2+bx+c$ où $a\neq 0$ et $a,b,c\in \mathbb{R}$, sa courbe représentative s'appelle une {\bfseries parabole}. Une parabole peut s'écrire sous différentes formes : forme générale, forme canonique, forme factorisée.
\end{defi}
...
\begin{defi}[sommet]
	Le {\bfseries sommet} d'une parabole, noté $S$, est le point de la parabole qui a l'ordonnée minimale (si $a>0$, convexe $\cup$) et maximale (si $a<0$, concave $\cap$). 
\end{defi}
...
\end{rem}
    ...
    \begin{defi}[Axe de symétrie d'une parabole]
	    L'{\bfseries axe de symétrie} d'une parabole $f$ est la droite d'équation $x=s$ tel que $f(s+t)=f(s-t), \forall t\in \R$. Elle partage la parabole en deux parties symétriques. 
    \end{defi}
    \begin{rem}
    ...
    \begin{defi}[forme générale d'une parabole]
	    La {\bfseries forme générale} d'une parabole est donnée par \[f(x)=ax^2+bx+c.\]
    \end{defi}
    
    \begin{defi}[forme canonique d'une parabole]
	    La {\bfseries forme canonique} d'une parabole est donnée par \[f(x)=a(x-\alpha)^2+\beta\] où $(\alpha;\beta)$ est le sommet de la parabole.
    \end{defi}

    \begin{defi}[forme factorisée d'une parabole]
	    La {\bfseries forme factorisée} d'une parabole est donnée par \[f(x)=a(x-x_1)(x-x_2)\] où $x_1$ et $x_2$ sont les zéros de la parabole.
    \end{defi}
	...
	\begin{enumerate}
        ...
        \[f(x)=\left(x-\left(-\dfrac{b}{2a}\right)\right)+f\left(-\dfrac{b}{2a}\right)\]
\item Complétion du carré :
	...
	&f(x)=a\left(x-\left(-\dfrac{b}{2a}\right)\right)^2 +\left( -\dfrac{b^2 -4ac}{4a}\right)\\
        \end{align*}
	\end{enumerate}
    \end{rem}
    \begin{rem}
...
Si $\Delta\geq 0$. On a $x_1=\dfrac{-b-\sqrt{b^2-4ac}}{2a}$ et $x_2=\dfrac{-b+\sqrt{b^2-4ac}}{2a}$. Ainsi, la forme factorisée est donnée par \[f(x)=a(x-x_1)(x-x_2).\]
    \end{rem}
    \begin{boiteExT}[Étude de la parabole $f(x)=3x^2+2x-1$]
	\vspace{10cm}
\end{boiteExT}
\begin{exo}
