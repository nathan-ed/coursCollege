...
\node (e34) [startstop, below of=e23, yshift=-1cm, xshift=0.5cm] {\centerline{Essayer de reconstituer} \centerline{l'identité \enquote{somme-produit}}\centerline{$\bm{(cx+a)(cx+b)}$}\centerline{(c.f. ci-dessous)}};
...
\begin{tasks}
	\task[] Dans l'expression \[9x^2+6xy+y^2,\]  on reconnaît deux carrés ($9x^2$ et $y^2$). Le troisième terme pourrait être un double produit $6xy=2\cdot 3xy$. Cela fonctionne, car
\end{tasks}
\tikzset{
...
\node[xshift=3cm,above=0.7cm] {{\bfseries Identités 1 et 2}};
...
child{
	\node(2a)[below=3cm] {$a$};
	child{
		\node(2)[bag,below=3.5cm] {\emptybox};
		\node[below=3.9cm,xshift=3cm] {$\leftrightarrow$ déterminer $a$ et $b$ $\leftrightarrow$}; 
		\node(22)[bag,below=3.5cm, xshift=6cm] {\emptybox};
	}
	\node(2b)[bag,below=3cm,xshift=6cm] {$b$};
}
child{
	\node(3ab)[below=5.5cm,xshift=3cm]{vérifier que le terme restant vaut $2ab$};
	\node(3)[bag,below=6cm,xshift=3cm]{\emptybox};
	\node[bag,below=7.5cm,xshift=3cm]{$(a\pm b)^2$};
}
}
...
\node[xshift=3cm,above=0.5cm] {{\bfseries Identité 3}};
...
child{
	\node[below=4.5cm,xshift=3cm]{$(a-b)(a+b)$};
}
}
\node[bag,xshift=6cm] {$b^2$};
\draw[->] (1)--(2a);
